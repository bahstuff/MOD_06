...
function createHtmlProduct(indice, producto){
    ...
    const productTotal = createProductTotalPriceHtmlElement(producto);
    var div = document.createElement("div");
    div.setAttribute("class","content");
...
function createInputHtmlElement(producto, indice){
    ...
    input.addEventListener("change", (event) => {
        if(parseInt(event.target.value, 10)  > producto.stock){
            ...
            document.getElementById("productPrice").innerText = (producto.units*producto.price) +(producto.units*producto.price*producto.tax)/100 + divisa;
            habilitarBoton();
    });
    return input;
}

//CREAR TOTAL PRODUCTO_____________________________________________________________________________________!!!!!!!!!!!!!!
function createProductTotalPriceHtmlElement(){
    var totalProductPrice = document.createElement("p");
    totalProductPrice.setAttribute("id", "productPrice")
